...
alpha_infstart=-20;                     
alpha_infend=20;
alpha_range=alpha_infend-alpha_infstart;
res=2;                 %Discrete resolution (evaluation points per degree)
loop=(res*(alpha_range))+1;%loop iters; MATLAB loops start at 1 not 0

for k=1:loop
    alpha_inf=alpha_infstart+((k-1)/res);   %AoA increased by 1 res per iter
...
%fileID=fopen('LLMECOPY2_Data.txt','w');
